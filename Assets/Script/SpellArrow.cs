...
public class SpellArrow : MonoBehaviour
{
        ...
        for (int i = 0; i < skillMgr.SkillIndex.Count; i++)
        {
            for (int j = 0; j < spellBook.spell.Count; j++)
            {
                if (skillMgr.SkillIndex[i] == spellBook.spell[j].name)
                {
                    ...
                    if (spellBook.spell[j].name == "FirePunch")
                    {
                        skill[i].Profile.sprite = Resources.Load<Sprite>("FirePunch");
                        skill[i].coolTime = spellBook.spell[j].cool;
                        skill[i].type = spellBook.spell[j].type;
                        skill[i].maxCount = spellBook.spell[j].count;
                    }
                    if (spellBook.spell[j].name == "EnergyBall")
                    {
                        skill[i].Profile.sprite = Resources.Load<Sprite>("EnergyBall");
                        skill[i].coolTime = spellBook.spell[j].cool;
                        skill[i].type = spellBook.spell[j].type;
                        skill[i].maxCount = spellBook.spell[j].count;
                    }
                    if (spellBook.spell[j].name == "Meteor")
                    {
                        skill[i].Profile.sprite = Resources.Load<Sprite>("Meteor");
                        skill[i].coolTime = spellBook.spell[j].cool;
                        skill[i].type = spellBook.spell[j].type;
                        skill[i].maxCount = spellBook.spell[j].count;
                    }
                    if (spellBook.spell[j].name == "Blizard")
                    {
                        skill[i].Profile.sprite = Resources.Load<Sprite>("Blizard");
                        skill[i].coolTime = spellBook.spell[j].cool;
                        skill[i].type = spellBook.spell[j].type;
                        skill[i].maxCount = spellBook.spell[j].count;
                    }
                    if (spellBook.spell[j].name == "Shild")
                    {
                        skill[i].Profile.sprite = Resources.Load<Sprite>("Shild");
                        skill[i].coolTime = spellBook.spell[j].cool;
                        skill[i].type = spellBook.spell[j].type;
                        skill[i].maxCount = spellBook.spell[j].count;
                    }
                    ...
                    skill[i].isClear = 0;
                    skill[i].nowCoolTime = 0f;
                    skill[i].nowCount = 0;
                    skill[i].isSkill = false;
                }
            }
        }
    ...
    public void UseSkill(string key)
    {
        for (int i = 0; i < skill.Count; i++)
        {
            ...
            if (skill[i].nowCoolTime > 0f) continue;
            ...
            else
            {
                ...
                if (skill[i].isClear == skill[i].maxClear)
                {
                    ...
                    GameManager.instance.isOpen = false;
                    skill[i].nowCoolTime = skill[i].coolTime;
                    skill[i].nowCount = skill[i].maxCount;
                    skill[i].isSkill = true;
                }
            }
    ...
    public void CoolTime()
    {
        for (int i = 0; i < skill.Count; i++)
        {
            if (skill[i].nowCoolTime > 0f)
            {
                closeSkill(i);
                skill[i].isSpell = false;
                if (skill[i].nowCount <= 0)
                {
                    skill[i].nowCoolTime -= Time.deltaTime;
                    skill[i].isSkill = false;
                };
                if (skill[i].nowCoolTime <= 0 && skill[i].isClear == 0)
                {
                    for (int j = 0; j < skill[i].arrow.Length; j++)
                    {
                        skill[i].arrow[j].color = new Color32(255, 255, 255, 255);
